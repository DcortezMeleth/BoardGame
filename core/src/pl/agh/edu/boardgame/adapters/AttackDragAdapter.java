...
/**
 * Adapter odpowiedzalny za obsluge przesuwania tokenow armii.
 *
 * @author Bartosz
 */
public class AttackDragAdapter extends DragListener implements InputProcessor {

    /** Logger. */
    private final static Logger LOGGER = LogManager.getLogger(AttackDragAdapter.class);
    ...
    public AttackDragAdapter(final BoardGameMain game, final Configuration configuration) {
        this.game = game;
    ...
    public boolean touchDown(final InputEvent event, final float x, final float screenY, final int pointer, final int button) {
        ...
        for(Nation token1 : game.getActivePlayer().getStillAvailableTokens()) {
    ...
    public void dragStop(final InputEvent event, final float x, final float screenY, final int pointer) {
        ...
        for(Field field : game.getMap().getFields()) {
            if(field.getPolygon().contains(x, y) && field.isValidAttacker(game.getActivePlayer(), token, game)) {
                LOGGER.debug("Atak!!");
                if(token.getField() != null) {
                    token.getField().getAttackingArmy().remove(token);
                }
                field.addAttackingToken(token);
                attacked = true;
            }
        }
        if(!attacked) {
    ...
    public boolean touchDown(final int screenX, final int screenY, final int pointer, final int button) {
        if(game.getPhase() != BoardGameMain.TurnPhase.ATTACK) {
            return false;
        }
